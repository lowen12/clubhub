...
def index():
    ...
    if request.method == "POST":
        ...
        if "search" in request.form:
            ...
            search1= '%' + search+ '%'
            clubs = db.execute("SELECT DISTINCT id, name FROM clubs WHERE name LIKE ?", search1)
...
def register():
    ...
    if request.method == "POST":
        ...
        # Add a new club to the database
            ...
            db.execute("INSERT INTO ratings (club_id, rating, commitment) VALUES (?, ?, ?)",
                    club_id[0]["id"], request.form.get("rating"), request.form.get("commitment"))
